...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of SUP contract.
    ...
    uint256 public latestPeriodSUPPerSecond;
    ...
    /// @notice Record the sUP amount belong to MasterChefV3.
    uint256 public supAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, ISupV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    /// @param _SUP The SUP token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _SUP, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        SUP = _SUP;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the sup per second , period end time.
    /// @param _pid The pool pid.
    /// @return supPerSecond Sup reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 supPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            supPerSecond = (latestPeriodSUPPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the sup per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return supPerSecond Sup reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 supPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            supPerSecond =
                (latestPeriodSUPPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending SUP rewards.
    /// @dev The pending sUP amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingSUP(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (SUP.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, ISupV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's SUP allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotSUPNFT();
    ...
    /// @notice harvest sup from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Sup reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce supAmountBelongToMC.
        if (_token == address(SUP)) {
            unchecked {
                // In fact balance should always be greater than or equal to supAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= supAmountBelongToMC) {
                    balance -= supAmountBelongToMC;
                } else {
                    // This should never happend.
                    supAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce supAmountBelongToMC.
        if (token == address(SUP)) {
            unchecked {
                // In fact balance should always be greater than or equal to supAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= supAmountBelongToMC) {
                    balanceToken -= supAmountBelongToMC;
                } else {
                    // This should never happend.
                    supAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of sup injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer sup token from receiver.
        SUP.safeTransferFrom(receiver, address(this), _amount);
        // Update supAmountBelongToMC
        unchecked {
            supAmountBelongToMC += _amount;
        }
        ...
        uint256 supPerSecond;
        uint256 supAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingSUP = ((latestPeriodEndTime - currentTime) * latestPeriodSUPPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingSUP);
            supAmount += remainingSUP;
        }
        supPerSecond = (supAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodSUPPerSecond = supPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, supPerSecond, supAmount);
    }

    /// @notice Update sup reward for all the liquidity mining pool.
    ...
    /// @notice Update sup reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer SUP.
    /// @param _to The SUP receiver address.
    /// @param _amount Transfer SUP amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = SUP.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update supAmountBelongToMC
            unchecked {
                if (supAmountBelongToMC >= _amount) {
                    supAmountBelongToMC -= _amount;
                } else {
                    supAmountBelongToMC = balance - _amount;
                }
            }
            SUP.safeTransfer(_to, _amount);
        }
    }
