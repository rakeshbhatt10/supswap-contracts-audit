...
import '@supswap/v3-core/contracts/libraries/LowGasSafeMath.sol';
...
library SmartRouterHelper {
    ...
    bytes32 internal constant V2_INIT_CODE_HASH = 0xca853cbcb27fba7742f7bd808ddc839c3e26f9ae3d4448d91bcdea146581b0a2;
    ...
    bytes32 internal constant V3_INIT_CODE_HASH = 0x3e03ddab0aa29c12c46cd283f9cf8c6800eb7ea3c6530a382474bac82333f2e0;
    ...
    /// @param deployer The Sup V3 deployer contract address
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) public pure returns (ISupV3Pool) {
        return ISupV3Pool(computeAddress(deployer, getPoolKey(tokenA, tokenB, fee)));
    }

    /// @notice Returns the address of a valid Sup V3 Pool
    /// @param deployer The contract address of the Sup V3 deployer
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (ISupV3Pool pool) {
    ...
    function verifyCallback(address deployer, PoolKey memory poolKey)
        public
        view
        returns (ISupV3Pool pool)
    {
        pool = ISupV3Pool(computeAddress(deployer, poolKey));
