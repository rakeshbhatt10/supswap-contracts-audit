...
import '@supswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@supswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@supswap/v3-periphery/contracts/libraries/Path.sol';
import '@supswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@supswap/v3-core/contracts/interfaces/ISupV3Pool.sol';
import '@supswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ISupV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ISupV3Pool pool) {
        pool = ISupV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ISupV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ISupV3Pool(pool).slot0();
            }
