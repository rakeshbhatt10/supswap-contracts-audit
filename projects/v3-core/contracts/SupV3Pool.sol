...
import './interfaces/ISupV3PoolDeployer.sol';
import './interfaces/ISupV3Factory.sol';
...
import '@supswap/v3-lm-pool/contracts/interfaces/ISupV3LmPool.sol';

contract SUPV3Pool is ISupV3Pool {
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == ISupV3Factory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = ISupV3PoolDeployer(msg.sender).parameters();
    ...
    function mint(
        ...
        ISupV3MintCallback(msg.sender).supV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            ISupV3SwapCallback(msg.sender).supV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        ISupV3FlashCallback(msg.sender).supV3FlashCallback(fee0, fee1, data);
