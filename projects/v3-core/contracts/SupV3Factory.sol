...
/// @title Canonical Sup V3 factory
/// @notice Deploys Sup V3 pools and manages ownership and control over pool protocol fees
contract SupV3Factory is ISupV3Factory {
    /***/
    ...
    function createPool(
        ...
        pool = ISupV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ISupV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ISupV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ISupV3Pool(pool).setLmPool(lmPool);
    }
}