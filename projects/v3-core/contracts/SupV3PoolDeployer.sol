...
import './SupV3Pool.sol';

contract SupV3PoolDeployer is ISupV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(SUPV3Pool).creationCode));
    ...
    /// @param factory The contract address of the Sup V3 factory
    ...
    function deploy(
        ...
        pool = address(new SUPV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}